...
Scenario: Cadastro bem-sucedido do arquivo XML de um pesquisador
    ...
    When eu seleciono a opção de upload escolhendo o arquivo "paulo_borba.xml"
    Then eu consigo ver o pesquisador "Paulo Henrique Monteiro Borba" na lista de pesquisadores
...
Scenario: Upload de um arquivo em formato inválido
    Given que estou na página de pesquisadores
    And o pesquisador "Paulo Henrique Monteiro Borba" não está na lista de pesquisadores
    When eu seleciono a opção de upload escolhendo o arquivo "paulo_borba_error.xml"
...
Scenario: Atualizar informações de um pesquisador cadastrado no sistema
    ...
    When seleciono a opção de "Atualizar currículo" escolhendo o arquivo "paulo_borba_atualizado.xml" com as novas informações
